...
namespace TCS.SimpleOptionsUI.Editor {
    ...
    public class IntSettingDrawer : PropertyDrawer {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            ...
            var targetObj = targetProp.objectReferenceValue;
            if (targetObj) {
                List<string> intFieldNames = GetIntFieldNames(targetObj);

                if (intFieldNames.Count > 0) {
                    int selectedIndex = intFieldNames.IndexOf(variableProp.stringValue);
                    if (selectedIndex == -1) selectedIndex = 0;
                    selectedIndex = EditorGUI.Popup(variableRect, "Variable Name", selectedIndex, intFieldNames.ToArray());
                    variableProp.stringValue = intFieldNames[selectedIndex];
                }
                else {
                    EditorGUI.LabelField(variableRect, "No float variables found in target object");
                }
        ...
        List<string> GetIntFieldNames(Object targetObj) {
            List<string> intFieldNames = new();
            const BindingFlags bindingFlags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;
            ...
            if (targetObj is GameObject go) {
                ...
                foreach (var comp in components) {
                    ...
                    IEnumerable<string> fields = compType.GetFields(bindingFlags)
                        ...
                        .Select(field => compType.Name + "/" + field.Name);
                    intFieldNames.AddRange(fields);

                    // Get properties
                    IEnumerable<string> properties = compType.GetProperties(bindingFlags)
                        .Where(prop => prop.PropertyType == typeof(int))
                        .Select(prop => compType.Name + "/" + prop.Name);
                    intFieldNames.AddRange(properties);
                }
            }
            else {
                // Handle ScriptableObjects and Components
